...
namespace LeastSquares.Undertone
{
    ...
    public class SpeechEngine : MonoBehaviour
    {
        ...
        public void SetLang(string newLanguage)
        {
            ...
            Loaded = false; // Mark the model as not loaded
            StartCoroutine(ReloadModel());
        }

        private IEnumerator ReloadModel()
        {
            Dispose(); // Dispose of the existing model and engine
            yield return LoadModel(); // Load the model with the new language
        }
    }
}